...
mat_OE.path = (list.files(paste0(directory, resol), pattern = "orca_predictions_32Mb.txt", recursive = TRUE, full.names = TRUE)) %>% 
  .[order(nchar(.), .)]

mat_Obs.path = (list.files(paste0(directory, resol), pattern = "orca_normmats_32Mb.txt", recursive = TRUE, full.names = TRUE)) %>% 
  .[order(nchar(.), .)]
...
WT_OE.lst = readOrcaMat(
  path_predicted_mat = paste0(directory, "wildtype/orca_predictions_32Mb.txt")
)

# Obs
WT_Obs.lst = readOrcaMat(
  path_predicted_mat = paste0(directory, "wildtype/orca_predictions_32Mb.txt"),
  path_norm_mat = paste0(directory, "wildtype/orca_normmats_32Mb.txt"),
  output = "Obs"
)
```
...
for (MAT in 1:length(MUT_OE.lst)) {
  ...
  # Calculate the ratio
  compRatio = (log2(mutCompIS.df$comp_local_IS[MAT] / wtISlocal.df$WT_local_IS[76]))
...
scores.plot = ggplot(plot.scores.df, aes(mutation_start, scores, col = score_type)) +
  ...
  geom_vline(xintercept = 9621000, col = "red", linetype = "dotted") +
  geom_vline(xintercept = 8320000, col = "black", linetype = "dotted") +
  geom_vline(xintercept = 20224000, col = "black", linetype = "dotted") +
  geom_vline(xintercept = 31232000, col = "black", linetype = "dotted")
scores.plot
```
...
analysed_IS.lst = list(WT = wtISglobal.gr, MUT66 = mutISglobal.lst$MUT66_global_IS, MUT76 = mutISglobal.lst$MUT76_global_IS, MUT159 = mutISglobal.lst$MUT159_global_IS, MUT245 = mutISglobal.lst$MUT245_global_IS)

# Create a list with all the PC1 we want to plot
analysed_PC1.lst = list(WT = wtPC1.gr, MUT66 = mutPC1.lst$MUT66_PC1, MUT76 = mutPC1.lst$MUT76_PC1, MUT159 = mutPC1.lst$MUT159_PC1, MUT245 = mutPC1.lst$MUT245_PC1)

# Create a list with all the comp we want to plot
analysed_comp.lst = list(MUT66 = compMUT.lst$MUT66_compartments, MUT76 = compMUT.lst$MUT76_compartments, MUT159 = compMUT.lst$MUT159_compartments, MUT245 = compMUT.lst$MUT245_compartments)
```
...
analysed_Obs_MAT.lst = list(MUT66 = MUT_Obs.lst$MUT66_Obs, MUT76 = MUT_Obs.lst$MUT76_Obs, MUT159 = MUT_Obs.lst$MUT159_Obs, MUT245 = MUT_Obs.lst$MUT245_Obs)

# List with Obs / Exp matrices
analysed_OE_MAT.lst = list(MUT66 = MUT_OE.lst$MUT66_OE, MUT76 = MUT_OE.lst$MUT76_OE, MUT159 = MUT_OE.lst$MUT159_OE, MUT245 = MUT_OE.lst$MUT245_OE)
```
...
```{r}
zoomMAT_Obs.lst = lapply(1:(length(analysed_Obs_MAT.lst) - 1), function(MAT) {
  matObs = mMATplot(
    matrix.upper = analysed_Obs_MAT.lst[[MAT]]$Matrix,
    matrix.lower = WT_Obs.lst$Matrix,
    start = analysed_Obs_MAT.lst[[MAT]]$parameters$mutation_start - 6e6,
    stop = analysed_Obs_MAT.lst[[MAT]]$parameters$mutation_end + 6e6,
    bin.width = analysed_Obs_MAT.lst[[MAT]]$parameters$bin_width,
    matrix.upper.txt = "MUT",
    matrix.lower.txt = "WT",
    scale.colors = "H",
...
```{r}
zoomMAT_OE.lst = lapply(1:(length(analysed_OE_MAT.lst) - 1), function(MAT) {
  matOE = mMATplot(
    matrix.upper = analysed_OE_MAT.lst[[MAT]]$Matrix,
    matrix.lower = WT_OE.lst$Matrix,
    start = analysed_OE_MAT.lst[[MAT]]$parameters$mutation_start - 6e6,
    stop = analysed_OE_MAT.lst[[MAT]]$parameters$mutation_end + 6e6,
    bin.width = analysed_OE_MAT.lst[[MAT]]$parameters$bin_width,
    matrix.upper.txt = "MUT",
    matrix.lower.txt = "WT",
    scale.colors = "OE",
...
```{r}
zoomMAT_FC.lst = lapply(1:(length(analysed_Obs_MAT.lst) - 1), function(MAT) {
  matFC = MATplot(
    matrix = analysed_Obs_MAT.lst[[MAT]]$Matrix / WT_Obs.lst$Matrix,
    start = analysed_Obs_MAT.lst[[MAT]]$parameters$mutation_start - 6e6,
    stop = analysed_Obs_MAT.lst[[MAT]]$parameters$mutation_end + 6e6,
...
## Zoom on the 245th mutation

```{r}
# Obs matrix
mut245_Obs.mat = mMATplot(
    matrix.upper = analysed_Obs_MAT.lst$MUT245$Matrix,
    matrix.lower = WT_Obs.lst$Matrix,
    start = 20e6,
    stop = 32e6,
    bin.width = analysed_Obs_MAT.lst$MUT245$parameters$bin_width,
    matrix.upper.txt = "MUT",
    matrix.lower.txt = "WT",
    scale.colors = "H",
    tad.upper.line = analysed_comp.lst$MUT245,
    tad.lower.line = compWT.gr) +
    ggtitle("Zoom MUT245 & WT Obs matrix",
            subtitle = paste0("Mutation from ", analysed_Obs_MAT.lst$MUT245$parameters$mutation_start, " Mb to ",
                              analysed_Obs_MAT.lst$MUT245$parameters$mutation_end, " Mb")) +
    geom_hline(yintercept = -analysed_Obs_MAT.lst$MUT245$parameters$mutation_start, col = "black", linetype = "dotted") +  # Location of the mutation
    geom_hline(yintercept = -analysed_Obs_MAT.lst$MUT245$parameters$mutation_end, col = "black", linetype = "dotted") +
    geom_vline(xintercept = analysed_Obs_MAT.lst$MUT245$parameters$mutation_start, col = "black", linetype = "dotted") +
    geom_vline(xintercept = analysed_Obs_MAT.lst$MUT245$parameters$mutation_end, col = "black", linetype = "dotted") +
    geom_hline(yintercept = -9621000, col = "darkgoldenrod4", linetype = "dotted") +  # Location of the compartment edge
    geom_vline(xintercept = 9621000, col = "darkgoldenrod4", linetype = "dotted")

# Obs / Exp matrix
mut245_OE.mat = mMATplot(
    matrix.upper = analysed_OE_MAT.lst$MUT245$Matrix,
    matrix.lower = WT_OE.lst$Matrix,
    start = 20e6,
    stop = 32e6,
    bin.width = analysed_OE_MAT.lst$MUT245$parameters$bin_width,
    matrix.upper.txt = "MUT",
    matrix.lower.txt = "WT",
    scale.colors = "OE",
    tad.upper.line = analysed_comp.lst$MUT245,
    tad.lower.line = compWT.gr) +
    ggtitle("Zoom MUT245 & WT Obs / Exp matrix",
            subtitle = paste0("Mutation from ", analysed_OE_MAT.lst$MUT245$parameters$mutation_start, " Mb to ",
                              analysed_OE_MAT.lst$MUT245$parameters$mutation_end, " Mb")) +
    geom_hline(yintercept = -analysed_OE_MAT.lst$MUT245$parameters$mutation_start, col = "black", linetype = "dotted") +  # Location of the mutation
    geom_hline(yintercept = -analysed_OE_MAT.lst$MUT245$parameters$mutation_end, col = "black", linetype = "dotted") +
    geom_vline(xintercept = analysed_OE_MAT.lst$MUT245$parameters$mutation_start, col = "black", linetype = "dotted") +
    geom_vline(xintercept = analysed_OE_MAT.lst$MUT245$parameters$mutation_end, col = "black", linetype = "dotted") +
    geom_hline(yintercept = -9621000, col = "darkgoldenrod4", linetype = "dotted") +  # Location of the compartment edge
    geom_vline(xintercept = 9621000, col = "darkgoldenrod4", linetype = "dotted")

# Fold Change matrix
mut245_FC.mat = MATplot(
    matrix = analysed_Obs_MAT.lst$MUT245$Matrix / WT_Obs.lst$Matrix,
    start = 20e6,
    stop = 32e6,
    bin.width = analysed_Obs_MAT.lst$MUT245$parameters$bin_width,
    scale.colors = "OE2",
    tad.upper.line = analysed_comp.lst$MUT245,
    tad.lower.line = analysed_comp.lst$MUT245) +
    ggtitle("Zoom MUT245 & WT Fold change matrix",
            subtitle = paste0("Mutation from ", analysed_Obs_MAT.lst$MUT245$parameters$mutation_start, " Mb to ",
                              analysed_Obs_MAT.lst$MUT245$parameters$mutation_end, " Mb")) +
    geom_hline(yintercept = -analysed_Obs_MAT.lst$MUT245$parameters$mutation_start, col = "black", linetype = "dotted") +  # Location of the mutation
    geom_hline(yintercept = -analysed_Obs_MAT.lst$MUT245$parameters$mutation_end, col = "black", linetype = "dotted") +
