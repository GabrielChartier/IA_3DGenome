...
scores.plot = ggplot(plot.scores.df, aes(mutation_start, scores, col = score_type)) +
  ...
  geom_vline(xintercept = 7701000, col = "red", linetype = "dotted") +
  geom_vline(xintercept = 6464000, col = "black", linetype = "dotted") +
  geom_vline(xintercept = 8384000, col = "black", linetype = "dotted")
scores.plot
```
...
analysed_IS.lst = list(WT = wtISglobal.gr, MUT102 = mutISglobal.lst$MUT102_global_IS, MUT121 = mutISglobal.lst$MUT121_global_IS, MUT132 = mutISglobal.lst$MUT132_global_IS)

# Create a list with all the PC1 we want to plot
analysed_PC1.lst = list(WT = wtPC1.gr, MUT102 = mutPC1.lst$MUT102_PC1, MUT121 = mutPC1.lst$MUT121_PC1, MUT132 = mutPC1.lst$MUT132_PC1)

# Create a list with all the comp we want to plot
analysed_comp.lst = list(MUT102 = compMUT.lst$MUT102_compartments, MUT121 = compMUT.lst$MUT121_compartments, MUT132 = compMUT.lst$MUT132_compartments)
```
...
analysed_Obs_MAT.lst = list(MUT102 = MUT_Obs.lst$MUT102_Obs, MUT121 = MUT_Obs.lst$MUT121_Obs, MUT132 = MUT_Obs.lst$MUT132_Obs)

# List with Obs / Exp matrices
analysed_OE_MAT.lst = list(MUT102 = MUT_OE.lst$MUT102_OE, MUT121 = MUT_OE.lst$MUT121_OE, MUT132 = MUT_OE.lst$MUT132_OE)
```
...
MAT_Obs.lst = lapply(1:length(analysed_Obs_MAT.lst), function(MAT) {
  matObs = mMATplot(
    ...
    geom_hline(yintercept = -7701000, col = "darkgoldenrod4", linetype = "dotted") +  # Location of the compartment edge
    geom_vline(xintercept = 7701000, col = "darkgoldenrod4", linetype = "dotted")
})
MAT_Obs.lst
```
...
zoomMAT_OE.lst = lapply(1:(length(analysed_OE_MAT.lst)), function(MAT) {
  matOE = mMATplot(
    matrix.upper = analysed_OE_MAT.lst[[MAT]]$Matrix,
    matrix.lower = WT_OE.lst$Matrix,
    start = (analysed_OE_MAT.lst[[MAT]]$parameters$mutation_start - analysed_OE_MAT.lst[[MAT]]$parameters$start) - 6e6,
    stop = (analysed_OE_MAT.lst[[MAT]]$parameters$mutation_end - analysed_OE_MAT.lst[[MAT]]$parameters$start) + 6e6,
    bin.width = analysed_OE_MAT.lst[[MAT]]$parameters$bin_width,
    matrix.upper.txt = "MUT",
    matrix.lower.txt = "WT",
    scale.colors = "OE",
