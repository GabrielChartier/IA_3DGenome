...
library(devtools)
source("~/Documents/Stage projet/Stage_Gabriel/Fonctions/_R/Function_readOrcaMatrix.R")
source("~/Documents/Stage projet/Stage_Gabriel/Fonctions/_R/Function_SIC_1mat.R")
source("~/Documents/Stage projet/Stage_Gabriel/Fonctions/_R/Function_IS_Predicted_and_Observed.R")
source("~/Documents/Stage projet/Stage_Gabriel/Fonctions/_R/Function_IS.R")
source("~/Documents/Stage projet/Stage_Gabriel/Fonctions/_R/Function_localSIC.R")
```

# Parameters

```{r}
directory = "~/Documents/Stage projet/Stage_Gabriel/Orca_Thomas/screening/"
saveDir = "~/Documents/Stage projet/Stage_Gabriel/Results/"
resol = "64kb/"
output_type = "Obs"
...
wtISglobal.df = insScoreObserved(
...
```{r}
wtISlocal.num = VPinsScore(
  matrix = WT_Obs.lst$Matrix,
  view.point = WT_Obs.lst$parameters$mpos)
```
...
for (MAT in 1:length(MUT_OE.lst)) {
     ...
     tmp_lIS.num = VPinsScore(
       matrix = MUT_Obs.lst[[MAT]]$Matrix,
       view.point = MUT_Obs.lst[[MAT]]$parameters$mpos)
     
     # Create a df tmp to store the main
     tmp_lIS.df = data.frame(chr = 1, mutation_start = MUT_Obs.lst[[MAT]]$parameters$mutation_start,
                             mutation_end = MUT_Obs.lst[[MAT]]$parameters$mutation_end, local_IS = tmp_lIS.num)
     ...
     # Create the .txt file
     write.table(mutISlocal.df, paste0(saveDir, resol, "local_IS.txt"), row.names = TRUE, sep = "\t")
...
for (MAT in 1:length(MUT_OE.lst)) {
  ...
  # Calculate the ratio
  ratio = (log2(mutISlocal.df$local_IS[MAT] / wtISlocal.num))
  ...
  # Create the .txt file
  write.table(ratioIS.df, paste0(saveDir, resol, "ratio_IS.txt"), row.names = TRUE, sep = "\t")
  
  
  ##### Calculate the local Structural Impact Score of the mutation #####
  ...
  tmp_SIC.df = data.frame(chr = 1, mutation_start = MUT_Obs.lst[[MAT]]$parameters$mutation_start,
                              mutation_end = MUT_Obs.lst[[MAT]]$parameters$mutation_end, local_SIC = sic.num)
  ...
  # Create the .txt file
  write.table(SIC.df, paste0(saveDir, resol, "local_SIC.txt"), row.names = TRUE, sep = "\t")
  ...
  tmp_MATcorr.df = data.frame(chr = 1, mutation_start = MUT_Obs.lst[[MAT]]$parameters$mutation_start,
                              mutation_end = MUT_Obs.lst[[MAT]]$parameters$mutation_end, MATcorr = MATcorr.bg[2,1])
...
scores.df = merge(mutISlocal.df, ratioIS.df)
...
plot.scores.df = data.frame(mutation_start = scores.df$mutation_start,
                            scores = c(scores.df$local_IS, scores.df$ratio_IS, scores.df$local_SIC, scores.df$PC1corr, scores.df$MATcorr),
                            score_type = c(rep("local_IS", nrow(scores.df)),
                                           rep("log2(ratioIS)", nrow(scores.df)),
                                           rep("local_SIC", nrow(scores.df)),
...
scores.plot = ggplot(plot.scores.df, aes(mutation_start, scores, col = score_type)) +
  ...
  facet_grid(~factor(score_type, levels = c("local_IS", "log2(ratioIS)", "local_SIC", "MATcorr", "PC1corr")) ~ ., scales = "free")
scores.plot
```
...
TAD.bed = read.table("~/Documents/Stage projet/Stage_Gabriel/Datas_tuto/tad3654_10kb.bed")
...
analysed_IS.lst = list(WT = wtISglobal.gr, MUT40 = mutISglobal.lst$MUT40_global_IS, MUT119 = mutISglobal.lst$MUT119_global_IS, MUT120 = mutISglobal.lst$MUT120_global_IS)

# Create a list with all the PC1 we want to plot
analysed_PC1.lst = list(WT = wtPC1.gr, MUT40 = mutPC1.lst$MUT40_PC1, MUT119 = mutPC1.lst$MUT119_PC1, MUT120 = mutPC1.lst$MUT120_PC1)

# Create a list with all the comp we want to plot
analysed_comp.lst = list(MUT40 = compMUT.lst$MUT40_compartments, MUT119 = compMUT.lst$MUT119_compartments, MUT120 = compMUT.lst$MUT120_compartments)
```
...
analysed_Obs_MAT.lst = list(MUT40 = MUT_Obs.lst$MUT40_Obs, MUT119 = MUT_Obs.lst$MUT119_Obs, MUT120 = MUT_Obs.lst$MUT120_Obs)

# List with Obs / Exp matrices
analysed_OE_MAT.lst = list(MUT40 = MUT_OE.lst$MUT40_OE, MUT119 = MUT_OE.lst$MUT119_OE, MUT120 = MUT_OE.lst$MUT120_OE)
```
...
zoomMAT_Obs.lst = lapply(1:length(analysed_Obs_MAT.lst), function(MAT) {
  matObs = mMATplot(
    matrix.upper = analysed_Obs_MAT.lst[[MAT]]$Matrix,
    matrix.lower = WT_Obs.lst$Matrix,
    start = analysed_Obs_MAT.lst[[MAT]]$parameters$mutation_start - 3e6,
    stop = analysed_Obs_MAT.lst[[MAT]]$parameters$mutation_end + 3e6,
    bin.width = analysed_Obs_MAT.lst[[MAT]]$parameters$bin_width,
    matrix.upper.txt = "MUT",
    matrix.lower.txt = "WT",
    scale.colors = "H",
...
zoomMAT_OE.lst = lapply(1:length(analysed_OE_MAT.lst), function(MAT) {
  matOE = mMATplot(
    matrix.upper = analysed_OE_MAT.lst[[MAT]]$Matrix,
    matrix.lower = WT_OE.lst$Matrix,
    start = analysed_OE_MAT.lst[[MAT]]$parameters$mutation_start - 3e6,
    stop = analysed_OE_MAT.lst[[MAT]]$parameters$mutation_end + 3e6,
    bin.width = analysed_OE_MAT.lst[[MAT]]$parameters$bin_width,
    matrix.upper.txt = "MUT",
    matrix.lower.txt = "WT",
    scale.colors = "OE",
